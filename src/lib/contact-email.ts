...
export function renderContactSupportEmail(message: ContactEmailMessage) {
  ...
  return {
    subject,
    html: `<!doctype html><html lang="en"><body style="font-family:'Times New Roman',Times,serif;color:#000"><main style="max-width:620px;margin:auto;padding:32px">${emailLogoHeader()}<h1 style="color:#3c3b6e">New contact inquiry</h1><p><strong>Name:</strong> ${escapeHtml(message.fullName)}</p><p><strong>Email:</strong> ${escapeHtml(message.email)}</p><p><strong>${escapeHtml(orderReference(message))}</strong></p>${warning}<h2 style="color:#3c3b6e">Message</h2><p style="white-space:pre-wrap">${escapeHtml(message.message)}</p></main></body></html>`,
...
export function renderContactCustomerReceipt(message: ContactEmailMessage) {
  ...
  return {
    ...
    html: `<!doctype html><html lang="en"><body style="font-family:'Times New Roman',Times,serif;color:#000"><main style="max-width:620px;margin:auto;padding:32px">${emailLogoHeader()}<h1 style="color:#3c3b6e">Thank you for contacting US Vital Certificates</h1><p>We received your message and a USVC representative will reply within one business day.</p><p><strong>${escapeHtml(reference)}</strong></p><p>Please do not send payment card numbers or Social Security numbers by email.</p><p style="color:#555;font-size:13px">USVC is an independent service and is not a government agency.</p></main></body></html>`,
