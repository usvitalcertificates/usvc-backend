...
test("order document stores requestorSsn as plaintext alongside form data", () => {
  const doc = new Order({
    ...
    signature: "T R",
    pricing: { serviceCents: 12500, bundleCents: 11300, rushCents: 0, totalCents: 23800 },
    amountCents: 23800,
  });
...
test("rejects 6 copies (max is 5)", () => {
  const schema = createOrderSchema.safeParse({ ...base("BIRTH"), ...SUBJECTS.BIRTH, copies: 6 });
