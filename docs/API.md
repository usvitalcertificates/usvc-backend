...
| POST   | `/auth/invite`                         | Super-admin only. Creates a pending STAFF account. Email-enabled envs queue a `STAFF_INVITATION` outbox email and return `{id, emailed:true}` (no token); otherwise returns a 48h `setupToken` for local setup.                                                       |
...
| GET    | `/staff/orders`                        | Masked fulfillment queue (`search`, `status`, `certificate`, `rushOnly`, `assigned=mine\|unassigned\|all`, `openOnly`, `attentionFirst`, `page`). Agents see unassigned paid + own; admins see all. `attentionFirst` sorts parked exceptions, then rush, then oldest. |
...
| GET    | `/staff/orders/:id`                    | Full detail for owner-agent or super-admin; SSN/card stay `*********`.                                                                                                                                                                                                |
| POST   | `/staff/orders/:id/notes`              | Internal note (1–2000 chars); never shown to customers.                                                                                                                                                                                                               |
| PATCH  | `/orders/:id/status`                   | Staff-only fulfillment advance (`IN_REVIEW`/`SUBMITTED` plus `ON_HOLD`/`NEED_INFO` with required `note`); owner-agent or super-admin; paid orders only. Landing on `SUBMITTED` queues one `SUBMISSION_NOTIFICATION` customer email (email-enabled envs).              |
| GET    | `/admin/staff`                         | Super-admin roster with workload counts (no secrets).                                                                                                                                                                                                                 |
| PATCH  | `/admin/staff/:id`                     | Super-admin renames or disables (disabling revokes sessions).                                                                                                                                                                                                         |
