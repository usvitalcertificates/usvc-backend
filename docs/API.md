...
| POST   | `/auth/invite`                         | Super-admin only. Creates a pending STAFF account; returns a 48h `setupToken`.                                                                                                                                  |
...
| GET    | `/staff/orders`                        | Masked fulfillment queue (`search`, `status`, `certificate`, `assigned=mine\|unassigned\|all`, `openOnly`, `page`). Agents see unassigned paid + own; admins see all.                                           |
...
| PATCH  | `/orders/:id/status`                   | Staff-only fulfillment advance (`IN_REVIEW`/`SUBMITTED` plus `ON_HOLD`/`NEED_INFO` with required `note`); owner-agent or super-admin; paid orders only.                                                         |
