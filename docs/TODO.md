...
- [x] Completion PDF 2026-09-25 (`feat/order-document-upload`): single PDF per order in GridFS (`order_docs`, 10 MB cap, `%PDF-` magic check, multer memory upload); `POST/GET/DELETE /orders/:id/document` owner-or-ADMIN with `document_uploaded/downloaded/deleted` audit events; SUBMITTED requires the PDF + a completion note from non-ADMIN (ADMIN bypasses); `order-document.ts` validators tested
