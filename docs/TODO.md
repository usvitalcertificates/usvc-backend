...
- [x] Staff password management 2026-09-25: ADMIN self-service `POST /auth/password` (current verified, 12+ chars, all sessions revoked, audited) plus admin-issued 48h setup-link resets `POST /admin/staff/:id/password-reset` for active staff (self-reset blocked); `/auth/setup` completes resets for active accounts with session revoke and distinct audit; 8+ char minimum everywhere
